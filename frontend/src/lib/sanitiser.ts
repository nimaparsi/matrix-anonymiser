...
export function sanitiseText(input: string, detectors: Record<DetectorKey, boolean>): SanitiseResult {
  ...
  replaceIf(
    detectors.secret,
    /\b(?:sk_(?:live|test)_[A-Za-z0-9]{12,}|ghp_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|AKIA[0-9A-Z]{16}|(?:api[_-]?key|access[_-]?token|private[_-]?key)\s*[:=]\s*[A-Za-z0-9._-]{12,})\b/gi,
    (match) => tokenFor('Secret', match),
  )

  replaceIf(detectors.email, /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b/g, (match) => tokenFor('Email', match))

  replaceIf(detectors.phone, /\b(?:\+?\d[\d\s().-]{7,}\d)\b/g, (match) => tokenFor('Phone', match))
  ...
  replaceIf(
    detectors.address,
    /\b\d{1,5}\s+[A-Z][A-Za-z]+(?:\s+[A-Z][A-Za-z]+){0,4}\s(?:Street|St|Road|Rd|Lane|Ln|Avenue|Ave|Drive|Dr|Way|Court|Ct|Close|Boulevard|Blvd)\b/g,
    (match) => tokenFor('Address', match),
  )

  replaceIf(
    detectors.username,
    /\b(?:user(?:name)?\s*[:=]\s*['"]?[a-zA-Z][a-zA-Z0-9._-]{2,}|['"][a-z][a-z0-9._-]{2,}['"])\b/gi,
    (match) => tokenFor('Username', match),
  )
  ...
  replaceIf(detectors.person, /\b([A-Z][a-z]+\s+[A-Z][a-z]+)\b/g, (match) => {
    if (match.startsWith('[')) return match

    const [firstWord, secondWord] = match.split(' ')
